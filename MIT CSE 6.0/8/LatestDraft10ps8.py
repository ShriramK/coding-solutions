	...
	if length == 0 or maxWork == 0:
		...
		if res[length, maxWork][VALUE] == -1:
			if difference > 0:
				...
				subResultOne = dpAdvisorHelper(works, values, length-1, maxWork, listOfSubjects)
				newMaxWork = maxWork - works[length-1]			
				subResultTwo = dpAdvisorHelper(works, values, length-1, newMaxWork, listOfSubjects)
				subResultTwo[VALUE] += values[length-1]			
				maxValue = max(subResultOne[VALUE], subResultTwo[VALUE])
				print 'maxValue ', maxValue, 'maxWork ', maxWork
				print 'subResultOne ',subResultOne, 'subResultTwo ', subResultTwo
				if maxValue == subResultTwo[VALUE]:
					bestSubSet.append(listOfSubjects[length-1])
					if listOfSubjects[length-1] == '9.00':
						print '#1 ' , bestSubSet					
