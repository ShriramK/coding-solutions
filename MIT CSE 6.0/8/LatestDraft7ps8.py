			...
			for item in newLists:#ret:
				...
				if tempWork < newMaxWork:
					...
					if totalWork <= newMaxWork:
						temp.append(subSet[VALUE])#subSet[0])
						#for items in item:
							#newCombs.append(items)
						if not res.__contains__(tuple(sorted(temp))):#newCombs))):
							if totalWork < newMaxWork:
								resizeSet.append(temp)#newCombs)
							#print 'res ' ,res
							#item changes and value of previous state is lost
							#tempValue = res[tuple(sorted(item))][VALUE]
							totalValue = tempValue + subSetValue
							newComb = tuple(sorted(temp))#newCombs))
...
for i in range(1,9):#(10,15):#15):#7):#20):#(31):#16):#(10):#(6):#(15)
	#print ' i ' , i
	"""
	print '******************************** '
