...
def dpAdvisorHelper( subSet, newList, newMaxWork, subjects ):
	...
	if subSet[0] == '7.17':
		print '\n7.17 found\n'
		print 'True/False ', res.__contains__(tuple([subject]))
		if not res.__contains__(tuple([subject])):
			print 'res ', res
	"""
	...
	if len(newList) > 1:
		...
		subSet = [subject]
		#print 'subSet ', subSet
		#print 'newList[WORK: ] ',newList[WORK: ] 
		newLists = dpAdvisorHelper( subSet, newList[WORK:], newMaxWork, subjects )
		"""
		if subSet[0] == '6.00':
			print 'ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ'
			print 'subSet[0] == 6.00'
			print 'newLists ' ,newLists, ' len ' , len(newLists)
			if newLists.__contains__(['7.17']):
				print 'Found 7.17 in the combinations\n'
		"""
		...
		if subSet[0] == '7.17':
			...
			for item in newLists:
				...
				if item == ['6.00']:
					print "\n\nif item == ['6.00']:"
					print 'Found 6.00 in the combinations'
					print 'item ', item 				
				if item.__contains__(['6.00']):
					print "if item.__contains__(['6.00']):"
					print 'Found 6.00 in the combinations\n'
					print 'item ', item 
		"""
		if newMaxWork - subjects[subSet[0]][WORK] > 0:#diff>0
			#print 'ret ', len(ret)
			for item in ret:
				newCombs = subSet[:]
		...
		secondValue = subjects[subSet[VALUE]][VALUE]
		secondWork = subjects[subSet[VALUE]][WORK]
...
for i in range(2):#7):#20):#(31):#16):#(10):#(6):#(15)
	#print ' i ' , i
	print '******************************** '
