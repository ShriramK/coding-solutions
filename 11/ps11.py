...
class RectangularRoom(object):
	...
	def cleanTileAtPosition(self, pos):
		...
		pos: a Position
		"""
		self.tileArray[pos.getX()][pos.getY()] = 0
		self.cleaned += 1
	...
	def isTileCleaned(self, m, n):
		...
		"""
		return self.tileArray[pos.getX()][pos.getY()] == 0
    ...
    def isPositionInRoom(self, pos):
		...
		xCoord = pos.getX()
		yCoord = pos.getY()
		return xCoord >= 0 and xCoord < self.width  and 
				yCoord >= 0 and yCoord < self.height
...
class BaseRobot(object):
    ...
    def __init__(self, room, speed):
		...
		self.p = position.getRandomPosition()
...
class Robot(BaseRobot):
    ...
    def updatePositionAndClean(self):
		...
		newDirection = -1
		while newPosition.getX() > self.room.width-1 or newPosition.getY() > self.room.height-1:
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
	...
	for robotObj in num_robots:
		robotObject = robot_type()
		ans = []
		for index in num_trials:
			subAns = []
			val = 0.0
			timeSteps = 0
...
avg = runSimulations(10, 1.0, 15, 20, 0.8, 30, Robot, False);
# === Provided function
