...
def dpAdvisorHelper( subSet, newList, newMaxWork, subjects ):
	...
	if len(newList) > 1:
		...
		print 'ret ', ret, 'newLists ', newLists		
		print 'res ', res, 
		print 'tuple(subSet) ' , tuple(subSet)
		"""
		#sys.exit(1)
		subSetWork = subjects[subSet[0]][WORK]#res[tuple(subSet)][WORK]
		subSetValue = subjects[subSet[0]][VALUE]#res[tuple(subSet)][VALUE]
		resizeSet = []
		diff = newMaxWork - subSetWork
		if diff >= 0:
			#if diff == 0:
				#if not res.__contains__(tuple([subSet[0]])):
					#res[tuple([subSet[0]])] = tuple([subSetValue, subSetWork])
			#else:#if diff > 0:
				#print 'ret ', len(ret)
			for item in ret:
				newCombs = subSet[:]
		...
		if subjectWork <= newMaxWork:
			...
			else: #subjectWork < newMaxWork
				#subSet already added to res in the last function call
...
for i in range(7):#20):#(31):#16):#(10):#(6):#(15)
	#print ' i ' , i
	print '******************************** '
